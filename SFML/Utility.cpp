...
void centerOrigin(sf::RectangleShape& shape)
{
	...
	shape.setOrigin(std::floor(bounds.left + bounds.width / 2.f), std::floor(bounds.top + bounds.height / 2));
}
...
{
	std::uniform_int_distribution<> distr(exclusiveMin, exclusiveMax );
	return distr(RandomEngine);
}
float randomFloat(float exclusiveMin, float exclusiveMax)
{
	std::uniform_real_distribution<> distr(exclusiveMin, exclusiveMax);
	return  std::round(distr(RandomEngine) * 100) / 100;
}
